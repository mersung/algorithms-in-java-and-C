...
//1 초	128 MB	55964	19883	13596	33.547%
//문제
//사악한 암흑의 군주 이민혁은 드디어 마법 구슬을 손에 넣었고, 
//그 능력을 실험해보기 위해 근처의 티떱숲에 홍수를 일으키려고 한다. 
//이 숲에는 고슴도치가 한 마리 살고 있다. 고슴도치는 제일 친한 친구인 비버의 굴로 
//가능한 빨리 도망가 홍수를 피하려고 한다.
//
//티떱숲의 지도는 R행 C열로 이루어져 있다. 비어있는 곳은 '.'로 표시되어 있고, 
//물이 차있는 지역은 '*', 돌은 'X'로 표시되어 있다. 비버의 굴은 'D'로, 
...
//물도 비버의 소굴로 이동할 수 없다.
//
//티떱숲의 지도가 주어졌을 때, 고슴도치가 안전하게 비버의 굴로 이동하기 위해 필요한 
//최소 시간을 구하는 프로그램을 작성하시오.
//
//고슴도치는 물이 찰 예정인 칸으로 이동할 수 없다. 즉, 
...
public class Practice3055 {
	...
	static LinkedList<int[]> goQueue = new LinkedList();
	static LinkedList<int[]> waterQueue = new LinkedList();
	static int answer = 0;
	...
	static void bfs() {
		...
		goQueue.add(new int[] {current[0], current[1], 0});

		while(!goQueue.isEmpty()) {
			spread();
			int size = goQueue.size();
			
			for(int i = 0; i<size; i++) {
				int[] curr = goQueue.poll();
				...
				for(int j = 0; j<4; j++) {
					...
					if(nx >= 0 && nx < n && ny >= 0 && ny < m) {
						if(board[nx][ny] == '.') {
							goQueue.add(new int[] {nx, ny, curr[2]+1});
							board[nx][ny] = 'S';
