...
//2 초	512 MB	8389	4188	3415	48.890%
//문제
...
//T는 입력의 맨 첫 줄에 주어진다.각 테스트 데이터는 두 개의 행으로 주어지는데, 첫 행에는 소설을 구성하는 장의 수를 나타내는 양의 정수 K (3 ≤ K ≤ 1,000,000)가 주어진다. 
	...
	public static void main(String[] args) throws IOException{
		...
		for(int i = 0; i<t; i++) {
			...
			for(int j = 0; j<arr.length; j++) {
				queue.add(Long.valueOf(arr[j]));
			}
			
			while(queue.size() > 1) {
				
				Long file1 = queue.poll();
				Long file2 = queue.poll();
				sum += (file1+file2);
