...
public class Practice2156 {
	
	static int n; //포도주 잔의 개수 < 10000 n^2 불가능
	static int[] glasses;
	static int[] dp;
	...
	public static void main(String[] args) throws IOException{
		...
		for(int i = 3; i<n+1; i++) { 
			//2번 연속 먹은것, 2번째 전과 지금 먹은 것, 지금 먹지 않은 것 중 최대값 정하기
			dp[i] = Math.max(Math.max(dp[i-3]+glasses[i-1]+glasses[i], dp[i-2]+glasses[i]), dp[i-1]);
		}
